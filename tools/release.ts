...
async function buildPackage() {
  if (program.build) {
    ...
    await execute(cmd).catch(error => {
      console.log(`Failed to build @nwx/mat ... ${error}`);
      return false;
    });
  }
  return true;
...
program
  ...
  .option('-b, --build', 'Build @nwx/mat')
  .option('-p, --publish', 'Publish @nwx/mat@latest')
  .option('-d, --dev', 'Publish @nwx/mat@dev')
  .parse(process.argv);
